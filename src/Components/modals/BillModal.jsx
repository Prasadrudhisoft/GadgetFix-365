...
<body>
    ...
    <div class="bill-body">
      ...
      <div class="info-grid">
        <div class="info-card">
          ...
          <div class="info-value">${escapeHtml(billNo)}</div>
        </div>
        <div class="info-card">
        ...
        <div class="info-card">
          ...
          <div class="info-value">${escapeHtml(bill.user_name || '—')}</div>
        </div>
        <div class="info-card">
          <div class="info-label">Mobile</div>
          <div class="info-value">${escapeHtml(bill.mobile_no || '—')}</div>
        </div>
        <div class="info-card">
          <div class="info-label">Device</div>
          <div class="info-value">${escapeHtml(bill.brand_name || '')} ${escapeHtml(bill.model_name || '—')}</div>
        </div>
        <div class="info-card">
          <div class="info-label">Parts Type</div>
          <div class="info-value" style="text-transform: capitalize;">${escapeHtml(bill.confirmed_type || '—')}</div>
        </div>
      </div>
      ...
      <div class="totals">
        ...
        <div class="total-row">
          <span>Tax (${escapeHtml(String(bill.tax_percent || 0))}%)</span>
          <span style="font-weight: 600; color: #d97706;">${formatCurrency(bill.tax_amount)}</span>
        </div>
  ...
  <div class="no-print" style="text-align: center; margin-top: 20px;">
    <button onclick="window.print()" style="background: #1d4ed8; color: white; border: none; padding: 12px 24px; border-radius: 8px; font-size: 14px; font-weight: 600; cursor: pointer; margin: 0 8px;">
      🖨️ Print Bill
    </button>
    <button onclick="window.close()" style="background: #6b7280; color: white; border: none; padding: 12px 24px; border-radius: 8px; font-size: 14px; font-weight: 600; cursor: pointer; margin: 0 8px;">
      ✕ Close
    </button>
  </div>
</body>
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} size="lg" showCloseButton={false}>
      {/* Header */}
      <div className="bg-gradient-to-r from-brand-dark to-brand px-4 py-4 sm:px-7 sm:py-6 text-white sticky top-0 z-10">
        <div className="flex justify-between items-center">
          <div>
            <h2 className="font-sora text-lg sm:text-xl font-extrabold mb-0.5 tracking-tight flex items-center gap-2">
              <i className="fas fa-file-invoice-dollar"></i> Your Bill
            </h2>
            <p className="font-manrope text-[12px] sm:text-[13px] opacity-85 font-medium">
          ...
          >
            {isDownloading ? (
              ...
              <>
                ...
                <span className="text-sm font-semibold">Download Bill</span>
              </>
            )}
          </button>
        </div>
        ...
        <button
          onClick={onClose}
          className="absolute top-3 right-3 sm:top-4 sm:right-4 w-[34px] h-[34px] rounded-full bg-white/20 flex items-center justify-center transition-all hover:bg-white/30 hover:rotate-90"
          aria-label="Close"
        >
      ...
      {/* Body */}
      <div
        className="p-4 sm:p-7 overflow-y-auto custom-scrollbar bg-white"
        style={{ maxHeight: 'min(calc(100dvh - 160px), calc(90vh - 160px))' }}
      >
        {isLoading ? (
          ...
          <div className="space-y-5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <InfoCard label="Bill ID" value={escapeHtml(bill.bill_id || '—')} mono />
              <InfoCard label="Date" value={formatDate(bill.created_at)} />
              <InfoCard label="Customer" value={escapeHtml(bill.user_name || '—')} />
              <InfoCard label="Mobile" value={escapeHtml(bill.mobile_no || '—')} />
              <InfoCard
                label="Device"
                value={`${escapeHtml(bill.brand_name || '')} ${escapeHtml(bill.model_name || '—')}`.trim()}
              />
              <InfoCard
                label="Parts Type"
                value={escapeHtml(bill.confirmed_type || '—')}
                capitalize
              />
            </div>
                ...
                <table className="w-full min-w-[280px]">
                  <thead className="bg-gray-50">
                    <tr>
                      ...
                      <th className="px-4 py-3 text-right text-[10px] font-extrabold text-gray-500 uppercase tracking-wide whitespace-nowrap">
                        Amount
                      </th>
                    </tr>
                  </thead>
                    ...
                    {services.length > 0 ? (
                      services.map((s, i) => (
                        <tr key={i} className="border-b border-gray-100 last:border-b-0 hover:bg-gray-50 transition-colors">
                          ...
                          <td className="px-4 py-3 text-gray-800 text-xs font-medium break-words">
