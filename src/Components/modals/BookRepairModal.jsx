...
export const BookRepairModal = ({
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.category_id) {
      showToast('⚠️ Please select a device category.', 'error');
      return;
    }
    if (!formData.brand_id) {
      showToast('⚠️ Please select a brand.', 'error');
      return;
    }
    ...
    if (files.length === 0) {
      showToast('⚠️ Please upload at least one photo of your device.', 'error');
      return;
    }
    if (files.length > 5) {
      showToast('⚠️ Maximum 5 photos allowed.', 'error');
      return;
    }
  ...
  return (
   <Modal isOpen={isOpen} onClose={onClose} size="lg" showCloseButton={false}>
      {/* Header */}
      <div className="bg-gradient-brand px-7 py-6 text-white">
      ...
      {/* Body */}
      <form onSubmit={handleSubmit} className="p-7 max-h-[calc(90vh-180px)] overflow-y-auto custom-scrollbar">
